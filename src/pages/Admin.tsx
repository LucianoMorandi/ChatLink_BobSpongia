...
const CLIENT_FOLDER = "bob_spongia";
const PAGE_SIZE = 5;
...
const Admin: React.FC = () => {
  ...
  const fetchProducts = async () => {
    const snap = await getDocs(collection(db, "products_bobspongia"));
    const list = snap.docs.map((doc) => ({ ...(doc.data() as Product), id: doc.id }));
    setProducts(list);
  };

  const fetchCategories = async () => {
    const snap = await getDocs(collection(db, "config_bobspongio"));
  ...
  const handleImageUpload = async (): Promise<string | null> => {
    ...
    formData.append("upload_preset", "bobspongio_unsigned");
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      await updateDoc(doc(db, "products_bobspongia", editingId), productData);
    } else {
      await addDoc(collection(db, "products_bobspongia"), productData);
    }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Eliminar producto?")) {
      await deleteDoc(doc(db, "products_bobspongia", id));
      fetchProducts();
    }
  };
  ...
  const handleAddCategory = async () => {
    ...
    const configRef = doc(db, "config_bobspongio", "global");
